...
The first command is a preflight with no network. The second explicitly authorizes up to five OpenRouter generations, requires the configured server-side key/model and uses a fixed 0.50 USD conservative journal allowance. It reuses the reference companion's bounded request and citation validation. The empty-evidence case abstains locally. There are no automatic retries, including after provider errors or failed assertions. Reusing the same journal and unchanged fixture inputs/model/prompt version rescans saved results without generating again; changing only lexical expected assertions also avoids a new model call.
...
Prompt versions now participate in synthesis-evaluation request IDs. Running an explicitly changed prompt version can therefore create new paid attempts even with the same journal; reserve budget for that comparison. A criteria-only change still rescans saved answers. Reports distinguish configured and recorded prompt versions; old drafts may have no recorded version. Keep baseline reports when comparing targeted follow-ups.

A targeted v2 follow-up passed the supported-procedure and injected-document cases (2/2). The first full 4/6 baseline remains unchanged; this is not a second full evaluation. A separate ambiguous-comment probe returned neutral sentiment with confidence 0.6: it met the no-invented-satisfaction expectation but failed the predeclared confidence-at-most-0.5 check. Confidence remains model-declared and non-calibrated; the report retains this discrepancy.
