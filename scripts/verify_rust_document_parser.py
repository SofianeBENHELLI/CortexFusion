...
def verify_document_parser(client, headers, domain):
    ...
    for raw, name, locator in [
        (b"\xef\xbb\xbf" + "Call operations 🧠.".encode(), "native.txt", "section"),
        (docx(), "native.docx", "paragraph"),
        (pdf(), "native.pdf", "page"),
    ]:
        url = upload(raw, name)
        assert client.post(url + "/process", headers=headers(sub="bob")).status_code == 403
        r = client.post(url + "/process", headers=headers())
        assert r.status_code == 200, r.text
        f = r.json()
        assert f["status"] == "succeeded" and f["attempts"] == 1, f
    ...
    for raw, name, error in [
        (b"\xff", "bad.txt", "INVALID_ENCODING"),
        (b"\x00", "bad.txt", "INVALID_TEXT"),
        (b"  \n", "blank.md", "NO_EXTRACTABLE_TEXT"),
        (b"x" * 30001, "big.txt", "TEXT_REQUIRES_SPLITTING"),
        (b"raw", "unknown.bin", "UNSUPPORTED_FORMAT"),
        (b"bad", "bad.pdf", "PARSE_FAILED"),
        (pdf(encrypted=True), "secret.pdf", "ENCRYPTED_DOCUMENT"),
        (pdf(blank=True), "blank.pdf", "NO_EXTRACTABLE_TEXT"),
        (pdf(blank=True, pages=101), "big.pdf", "DOCUMENT_LIMIT"),
        (docx("<!DOCTYPE foo>"), "unsafe.docx", "UNSAFE_XML"),
    ]:
        url = upload(raw, name)
        r = client.post(url + "/process", headers=headers())
        assert r.status_code == 200, r.text
        f = r.json()
        assert f["status"] == "failed" and f["error_code"] == error and f["spans"] == [], (name, f)
        assert client.post(url + "/process", headers=headers()).json() == f
    ...
    url = upload(b"cancelled", "cancel.txt")
    client.post(url + "/cancel", headers=headers())
    assert client.post(url + "/process", headers=headers()).status_code == 409
    return [
        "native_rust_child_txt_docx_pdf_unicode_spans_and_source_receipts",
        "native_document_encryption_format_size_encoding_failures_and_retry",
    ]
