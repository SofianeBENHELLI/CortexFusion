...
Le résultat MCP contient `structuredContent: {http_status, data}` et sa représentation JSON dans le contenu texte. Les erreurs ont `isError: true` et le code métier dans `data.error`. Le schéma de sortie distingue les statuts de succès documentés des erreurs 400–599 et décrit la demande de confirmation optionnelle pour un statut 428 ; les reçus d’erreur sont vérifiés contre ce schéma dans les tests. Les fichiers binaires sont rendus sous `{media_type, base64}`. Les clients historiques gardent leurs résultats de succès historiques. Leurs erreurs sont des contenus texte JSON avec `error` et `message` assainis ; les diagnostics internes et les valeurs de validation ne sont pas renvoyés. Les nouveaux outils utilisent l’enveloppe uniforme ci-dessus.
