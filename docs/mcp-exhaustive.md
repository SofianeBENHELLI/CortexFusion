...
Les outils exécutent la route HTTP dans le même processus : application ASGI pour la référence Python, Router Axum pour Rust ; aucun client HTTP arbitraire ni nouvelle connexion réseau n'est utilisé pour joindre le backend. Les dépendances d'authentification, middleware, validations, statuts et services HTTP restent actifs. Les appels à un fournisseur IA depuis la route d'extraction restent naturellement des appels externes soumis à sa configuration.

Le résultat MCP contient `structuredContent: {http_status, data}` et sa représentation JSON dans le contenu texte. Les erreurs métier des outils ont `isError: true` et le code métier dans `data.error`. Un refus de transport ou une erreur JSON-RPC à la racine peut survenir sans résultat d’outil ; voir les [limites de transport et règles de reprise Rust](transport-limits.fr.md). Le schéma de sortie distingue les statuts de succès documentés des erreurs 400–599 et décrit la demande de confirmation optionnelle pour un statut 428 ; les reçus d’erreur sont vérifiés contre ce schéma dans les tests. Les fichiers binaires sont rendus sous `{media_type, base64}`. Les clients historiques gardent leurs résultats de succès historiques. Dans la référence Python, leurs erreurs sont des contenus texte JSON avec `error` et `message` assainis ; Rust utilise aussi l’enveloppe structurée `{http_status, data}` en erreur pour les alias. Les diagnostics internes et les valeurs de validation ne sont pas renvoyés. Les nouveaux outils utilisent l’enveloppe uniforme ci-dessus.
