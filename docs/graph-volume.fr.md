...
Le profil supplémentaire `--source-profile shared_long` conserve ces concepts et leurs droits mais les rattache à deux sources partagées de 200000 points de code chacune : une publique au lecteur et une privée au propriétaire. Les extraits sont distribués dans chaque texte, avec du remplissage Unicode et des preuves se terminant à la position 199999. Ce profil mesure le coût des références dans des documents longs ; il ne passe pas par l’import de fichiers ou l’extraction par modèle. Le nombre de points de code reste fixe, mais le nombre d’octets UTF-8 varie avec la proportion de remplissage Unicode et de preuves : le rapport expose cette taille, qui doit être prise en compte dans toute comparaison entre volumes.
...
Les fiches et questions alternent maintenant entre trois concepts publics, situés au début, au milieu et à la fin de l’ordre des concepts. L’oracle compare le hash de la source entière et les positions exactes de chaque extrait ; une citation valide du mauvais passage échoue. Le premier run historique ne variait pas ces cibles : une comparaison chronométrique entre les deux versions du protocole doit tenir compte de cette différence.

## Interpréter les mesures

Chaque lecture de graphe nécessite le snapshot complet, même pour une fiche concept. Les lectures simultanées peuvent partager son téléchargement et sa validation selon le [protocole de regroupement](concurrent-graph-reads.fr.md) ; les réponses filtrées restent individuelles et aucun résultat terminé n’est mis en cache. Une question ajoute la lecture des sources, le classement lexical et l’écriture d’un épisode sous verrou de domaine. Sa phase finale peut donc se sérialiser entre lecteurs du même domaine. Aucun appel modèle n’est effectué. Les deux profils sont enregistrés séparément ; le profil court ne permet pas de déduire le coût du profil à documents partagés.

La durée individuelle s’arrête après réception du corps HTTP, avant les comparaisons Python. Par défaut (`identity_mode=per_request`), les jetons synthétiques sont générés par tentative, avant ce chronométrage. Leur signature peut néanmoins espacer les admissions et consommer du temps processeur côté client. Le profil distinct `--identity-mode per_cell` signe un jeton frais avant chaque cellule et le réutilise pour ses requêtes, comme dans une session de compagnon. Une expiration éventuelle reste une erreur de qualification. La CI mesure aussi ce profil à 2000 concepts sur documents partagés. Ne pas comparer les deux modes comme un protocole identique. Le débit de cellule inclut l’orchestration et les contrôles du client. Médiane et p95 utilisent le rang le plus proche ; sur si peu d’échantillons, le p95 est voisin du maximum et n’est pas une estimation statistique stable. Les échantillons, codes de réponse et erreurs restent dans le rapport, sans retry masquant la première latence.
...
uv run python scripts/benchmark_rust_graph.py --synthetic-only \
  --build-profile release --source-profile shared_long \
  --output /tmp/cortex-graph-volume-long.json
```
...
Le mode `--simulate-engine --binary target/debug/cortex-rust-core --build-profile debug --sizes 100 --concurrency 1` sert à vérifier le protocole de fixture localement. Ses durées ne sont pas des performances TerminusDB et ne doivent pas être mélangées aux résultats réels. En simulation, chaque cellule expose aussi `controlled_engine_gets`, le nombre de GET observés par le moteur contrôlé pendant la cellule ; ce compteur n’est pas disponible dans les mesures directes du moteur réel. Le rapport distingue explicitement ces modes, l’empreinte du binaire, les versions, le SHA candidat et le merge de contrôle testé par la CI.

Le rapport est écrit avant et après chaque cellule pour conserver les résultats partiels. Un arrêt forcé peut laisser `status=running` : toujours rapprocher le JSON de la conclusion CI, sans traiter ce statut comme une réussite. Les jeux de données des volumes précédents restent présents pendant le run ; les mesures ne supposent pas un moteur réinitialisé entre volumes. La CI borne chaque profil à 15 minutes après compilation, dans un job limité à 45 minutes. Il reste nécessaire de qualifier d’autres formes de graphes, documents longs, tailles de corps/relations, nombreux tenants, fichiers, fournisseurs IA et conditions d’exploitation avant d’en déduire un dimensionnement.
