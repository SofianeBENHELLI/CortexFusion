...
A per-process semaphore allows one active extraction. A separate durable reservation ledger prevents duplicate calls for the same key across processes and enforces the configured domain daily attempt allowance. Failed/interrupted attempts have no successful extraction receipt; inspect their status before explicitly issuing a new command/key. Monetary organization budgets, durable background extraction jobs and provider invoice reconciliation remain future work. The adapter requests model unloading after generation; actual runtime behavior remains the local server's responsibility.
