...
Les champs `confirmation_policy` et les annotations MCP sont des instructions d'intégration pour l'hôte, pas des contrôles d'autorisation. Le backend n'implémente pas encore de reçu cryptographique prouvant une confirmation humaine. Il vérifie les rôles et les préconditions métier. Les outils MCP n'exposent volontairement pas l'approbation, la publication, les changements d'accès, la reconstruction ni les appels modèle payants. Un hôte propriétaire peut préparer ces actions à partir de la documentation HTTP et passer par son propre mécanisme de décision explicite avant d'invoquer l'API. Ne pas donner à un modèle un client HTTP arbitraire portant un jeton propriétaire.
...
Les interfaces sont remplaçables au niveau des contrats ; aucun frontend n'est créé dans cette étape. Les 16 outils couvrent des parcours de découverte, consultation, proposition et suivi personnel ; ils ne couvrent pas toutes les commandes administratives HTTP. Le client Cordis historique reste une interface limitée et ne constitue pas un SDK complet de toutes les opérations. Un consommateur plus large peut générer son client à partir d'OpenAPI.
