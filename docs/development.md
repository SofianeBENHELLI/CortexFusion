...
### Current evidence at publication

Global domain.publish and proposal-targeted publication both validate the accepted proposal's complete evidence access in the shared _publish transaction before projection writes. Owner membership and an earlier acceptance do not bypass a later evidence revocation. A denied publication leaves version, projection, outbox and publication audit unchanged; another owner retaining access may complete it. No-op global publication has no target evidence to inspect. Targeted historical retries retain their explicit target access check. Signed HTTP and generated MCP regression tests first reproduced the global bypass, then verify denial and authorized recovery.
